...
//  HSD
//
//  Created by Tô Tử Siêu on 6/26/18.
...
open class ISTimeline: UIScrollView {
    ...
    fileprivate static let gap:CGFloat = 30
    
    open var pointDiameter:CGFloat = 10.0 {
        didSet {
    ...
    fileprivate var sections:[(point:CGPoint, bubbleRect:CGRect, descriptionRect:CGRect?, titleLabel:UILabel, descriptionLabel:UILabel?, pointColor:CGColor, lineColor:CGColor,textColor : CGColor ,fill:Bool)] = []
    ...
    override open func draw(_ rect: CGRect) {
        ...
        for i in 0 ..< sections.count {
            ...
            drawBubble(sections[i].bubbleRect, backgroundColor: UIColor(cgColor: sections[i].textColor), textColor:UIColor.white, titleLabel: sections[i].titleLabel)
            
            let descriptionLabel = sections[i].descriptionLabel
            if (descriptionLabel != nil) {
                drawDescription(sections[i].descriptionRect!, textColor: UIColor(cgColor: sections[i].textColor), descriptionLabel: sections[i].descriptionLabel!)
            }
        }
    ...
    fileprivate func buildSections() {
        ...
        for i in 0 ..< points.count {
            ...
            sections.append((point, bubbleRect, descriptionRect, titleLabel, descriptionLabel, points[i].pointColor.cgColor, points[i].lineColor.cgColor,points[i].textColor.cgColor, points[i].fill))
